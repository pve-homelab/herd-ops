//! Top-level Dev Team TUI: split menu + sub-plugin host.
            ...
            Screen::Plugin { id } => {
                ...
                match action {
                    ...
                    NavAction::Back => {
                        ...
                        ctx.error = None;
                        screen = Screen::Menu {
                            selected: metas.iter().position(|m| m.id == id).unwrap_or(0),
                        };
                    }
    ...
    match screen {
        ...
        Screen::Plugin { id } => {
            ...
            draw_help_footer(
                frame,
                chunks[2],
                "Esc back to menu · q only quits from main menu",
            );
        }
    }
}
...
fn draw_menu(
    ...
    let header = Paragraph::new("Dev Team")
        .style(
            Style::default()
                .fg(Color::Cyan)
        ...
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title("Master gate · Orchestrator loop · live kanban"),
        );
