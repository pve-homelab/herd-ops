...
impl GitPlugin {
    ...
    fn default_clone_parent(ctx: &PluginCtx) -> String {
        ...
        let settings: Settings = read_toml(&ctx.paths.join_config("settings.toml"))
            ...
            .flatten()
            .unwrap_or_default();
        let chosen = if !settings.default_clone_dir.trim().is_empty() {
            settings.default_clone_dir.trim()
        } else {
            settings.default_workspace_dir.trim()
        };
        if !chosen.is_empty() {
            return expand_home(chosen);
        }
        std::env::var("HOME")
    ...
    /// Returns true when the caller should quit Dev Team so you land in that folder.
    ...
    fn sync_session(&self, ctx: &mut PluginCtx, cwd: &str, label: &str, remote: &str) {
        use crate::session::Session;
        let mut session = Session::load(&ctx.paths).unwrap_or_default();
        session.working_dir = cwd.to_string();
        session.project_name = label.to_string();
        if !remote.is_empty() {
            session.git_remote = remote.to_string();
        }
    ...
    fn open_workspace_at(&mut self, ctx: &mut PluginCtx, cwd: &str, label: &str) -> bool {
        let remote = self
            .repos
            .iter()
            .find(|r| r.name.ends_with(label) || r.name == label)
            .map(|r| r.clone_url.clone())
            .unwrap_or_default();
        self.sync_session(ctx, cwd, label, &remote);
        ...
        match run_herdr_ok(&[
            ...
            Ok(out) => {
                self.log = format!(
                    "Opened workspace at:\n  {cwd}\n\nSession cwd set for Master/Launch.\nClosing Dev Team so you can work there.\n{out}"
                );
...
impl SubPlugin for GitPlugin {
    ...
    fn description(&self) -> &'static str {
        "Clone a forge repo, set Master session cwd/project, open a focused Herdr workspace, and close Dev Team so the agents know where to work."
    }
            ...
            Mode::FormClone { repo_idx } => {
                ...
                match form.handle(key) {
                    ...
                    FormResult::Submit => {
                        ...
                        self.form = None;
                        self.mode = Mode::Browse;
                        // Quit closes the Dev Team popup after workspace create --focus.
