...
pub fn gather_checks(config_dir: &PathBuf, state_dir: &PathBuf) -> Vec<Check> {
    ...
    match run_herdr(&["plugin", "list", "--json"]) {
        Ok(out) if out.status.success() => {
            ...
            checks.push(Check {
                name: "herdr-board plugin (kanban)".into(),
                ok: true,
                detail: if has_board {
                    "Detected in plugin list".into()
                } else {
                    "Not installed — Kanban Board / Launch open_board will fail until installed".into()
                },
            });
            let _ = has_browser;
        }
    ...
    checks.push(Check {
        name: "session.toml".into(),
        ok: true,
        detail: if session_path.exists() {
            format!("present at {}", session_path.display())
        } else {
            "not created yet — Master / Lazy Git will write it".into()
        },
    });
...
impl SubPlugin for DoctorPlugin {
    ...
    fn description(&self) -> &'static str {
        "Herdr + agent + toolchain health check. Verifies binary, plugins, git, board, and Dev Team session state."
    }
