//! Paths and persistence for Dev Team config/state.
...
fn fallback_dir(kind: &str) -> PathBuf {
    if let Some(dirs) = ProjectDirs::from("", "herdr", "plugins") {
        let base = match kind {
            "state" => dirs.data_local_dir().join("dev-team"),
            _ => dirs.config_dir().join("dev-team"),
        };
        return base;
    }
    PathBuf::from(".").join(format!(".dev-team-{kind}"))
}
